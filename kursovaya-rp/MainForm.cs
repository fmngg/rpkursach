...
namespace kursovaya
{
    public partial class Form1 : Form
    {
        ...
        {
            string color = kursovaya.Class1.LightRed();
            int[] intArray = color.Split(',').Select(x => Convert.ToInt32(x)).ToArray();
            this.BackColor = Color.FromArgb(intArray[0], intArray[1], intArray[2]);
        }

        private void синийToolStripMenuItem_Click(object sender, EventArgs e)
        {
            string color = kursovaya.Class1.LightBlue();
            int[] intArray = color.Split(',').Select(x => Convert.ToInt32(x)).ToArray();
            this.BackColor = Color.FromArgb(intArray[0], intArray[1], intArray[2]);
        }
        ...
        {
            string color = kursovaya.Class1.LightGrey();
            int[] intArray = color.Split(',').Select(x => Convert.ToInt32(x)).ToArray();
            this.BackColor = Color.FromArgb(intArray[0], intArray[1], intArray[2]);
        }
        ...
        {            
            var saver = new WordSaver("Result.docx");
            var items = new Dictionary<string, string>
            {
                { "<NAME>", this.Text },
                { "<RESULT>", GetXYvalues(x, z, n).ToString() }
            };
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (helper.Open(filePath: Path.Combine(Environment.CurrentDirectory, DateTime.Now.ToString("yyyyMMdd HHmmss") +  "Result.xlsx")))
                    {
                        helper.Set(column: "A", row: 1, data: "Решение ДУ; разработал: Фомин");
                        helper.Set(column: "A", row: 2, data: GetXYvalues(x, z, n));
                        helper.Save();
                    }
                }
